...
from server.app import platform_app as app
from server.features.jobs.queue import QueueEnqueueResult
...
class PlatformModeQueueTests(unittest.TestCase):
    ...
    def test_submit_routes_use_shared_queue_path_when_rq_enabled(self) -> None:
        ...
        for scenario in scenarios:
            with self.subTest(path=scenario["path"]):
                with (
                    patch("server.features.jobs.queue.is_rq_enabled", return_value=True),
                    patch(
                        "server.features.jobs.queue.enqueue_platform_mode_submit_job",
                        return_value=QueueEnqueueResult(job_id="job-123", queue_name="analysis"),
                    ) as mock_enqueue,
                    patch("server.features.jobs.queue.record_platform_mode_submit_dispatch") as mock_dispatch,
                    patch("server.features.jobs.queue.record_platform_mode_enqueue_failure") as mock_enqueue_failure,
                ):
    ...
    def test_submit_routes_record_inline_dispatch_when_rq_disabled(self) -> None:
        scenarios = [
            {
                ...
                "mode": "auditor",
                "request_payload": {"problemId": "aud-1", "report": "report body"},
                "service_patch": "server.features.learning.api_auditor.submit_auditor_report",
                "service_response": {
                    "correct": True,
                    "score": 81.0,
                    "verdict": "passed",
            ...
            },
            {
                ...
                "mode": "refactoring-choice",
                "request_payload": {"problemId": "ref-1", "selectedOption": "A", "report": "report body"},
                "service_patch": "server.features.learning.api_refactoring_choice.submit_refactoring_choice_report",
                "service_response": {
                    "correct": True,
                    "score": 81.0,
                    "verdict": "passed",
            ...
            },
            {
                ...
                "mode": "code-blame",
                "request_payload": {"problemId": "blame-1", "selectedCommits": ["A"], "report": "report body"},
                "service_patch": "server.features.learning.api_code_blame.submit_code_blame_report",
                "service_response": {
                    "correct": True,
                    "score": 81.0,
                    "verdict": "passed",
            ...
            },
            {
                ...
                "mode": "single-file-analysis",
                "request_payload": {"problemId": "sfa-1", "report": "report body"},
                "service_patch": "server.features.learning.api_advanced_analysis.submit_advanced_analysis_report",
                "service_response": {
                    "correct": True,
                    "score": 81.0,
                    "verdict": "passed",
        ...
        for scenario in scenarios:
            with self.subTest(path=scenario["path"]):
                with (
                    patch("server.features.jobs.queue.is_rq_enabled", return_value=False),
