...
class PlatformModeJobStatusTests(unittest.TestCase):
    ...
    def test_returns_finished_job_result_for_owner(self) -> None:
        ...
        with (
            patch("server.features.jobs.api.is_rq_enabled", return_value=True),
            patch("server.features.jobs.api.get_redis_connection", return_value=object()),
            patch("server.features.jobs.api.Job.fetch", return_value=fake_job),
        ):
    ...
    def test_returns_404_for_missing_job(self) -> None:
        with (
            patch("server.features.jobs.api.is_rq_enabled", return_value=True),
            patch("server.features.jobs.api.get_redis_connection", return_value=object()),
            patch("server.features.jobs.api.Job.fetch", side_effect=NoSuchJobError("job-404")),
        ):
    ...
    def test_returns_400_when_queue_mode_is_not_rq(self) -> None:
        with patch("server.features.jobs.api.is_rq_enabled", return_value=False):
