...
from server.features.learning.auditor_service import create_auditor_problem, map_auditor_difficulty, submit_auditor_report
from server.features.learning.service import AUDITOR_TRAP_COUNT_BY_DIFFICULTY
from server.schemas.runtime import AuditorSubmitRequest
...
class AuditorSqlSubmitTests(unittest.TestCase):
    ...
    def test_submit_auditor_report_updates_submission_and_analysis(self):
        ...
        with (
            patch("server.features.learning.auditor_service._ai_client.analyze_auditor_report", return_value=evaluation),
            patch("server.features.learning.auditor_service.update_user_problem_stat") as mock_stat_update,
        ):
    ...
    def test_submit_ai_failure_returns_fallback_response(self):
        with (
            patch("server.features.learning.auditor_service._ai_client.analyze_auditor_report", side_effect=RuntimeError("ai down")),
            patch("server.features.learning.auditor_service.update_user_problem_stat") as mock_stat_update,
        ):
            result = submit_auditor_report(
                self.db,
    ...
    def test_create_problem_generation_failure_returns_user_friendly_error(self):
        ...
        with patch(
            "server.features.learning.auditor_service._generator.generate_auditor_problem_sync",
