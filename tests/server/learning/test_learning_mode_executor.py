...
from server.features.learning.executor import run_platform_mode_submit_background
...
class PlatformModeExecutorTests(unittest.TestCase):
    def test_auditor_background_submit_uses_bridge_for_runtime_problem_ids(self) -> None:
        ...
        with (
            patch("server.features.learning.executor.SessionLocal", return_value=fake_db),
            patch("server.features.learning.executor.observe_platform_mode_operation") as mock_observe,
            patch(
                "server.features.learning.executor.learning_service.submit_mode_answer",
    ...
    def test_deleted_modes_are_rejected(self) -> None:
        ...
        with (
            patch("server.features.learning.executor.SessionLocal", return_value=fake_db),
            patch("server.features.learning.executor.observe_platform_mode_operation") as mock_observe,
            patch("server.features.learning.executor.learning_service.submit_mode_answer") as mock_submit,
        ):
            mock_observe.return_value.__enter__.return_value = None
            mock_observe.return_value.__exit__.return_value = None
