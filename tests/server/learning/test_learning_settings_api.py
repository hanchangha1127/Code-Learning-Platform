...
class PlatformMeSettingsTests(unittest.TestCase):
    ...
    def test_get_settings_creates_defaults_when_row_missing(self) -> None:
        with (
            patch("server.features.account.api.get_settings", return_value=None) as mock_get_settings,
            patch(
                "server.features.account.api.update_settings",
    ...
    def test_invalid_preferred_language_is_ignored_for_problem_defaults(self) -> None:
        ...
        with patch("server.features.learning.api_problems.list_problems", return_value=([], 0)) as mock_list:
