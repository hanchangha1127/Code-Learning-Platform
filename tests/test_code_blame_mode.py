...
from app.services.code_blame_service import create_code_blame_problem, map_code_blame_difficulty, submit_code_blame_report
from backend.learning_mode_handlers import (
    CODE_BLAME_CANDIDATE_COUNT_BY_DIFFICULTY,
    CODE_BLAME_CULPRIT_COUNT_WEIGHTS,
)
from server_runtime.schemas import CodeBlameSubmitRequest
...
class CodeBlameSqlSubmitTests(unittest.TestCase):
    ...
    def test_submit_report_updates_submission_and_analysis(self):
        ...
        with (
            patch("app.services.code_blame_service._ai_client.analyze_code_blame_report", return_value=evaluation),
            patch("app.services.code_blame_service.update_user_problem_stat") as mock_stat_update,
        ):
    ...
    def test_submit_ai_failure_returns_fallback_response(self):
        with (
            patch("app.services.code_blame_service._ai_client.analyze_code_blame_report", side_effect=RuntimeError("ai down")),
            patch("app.services.code_blame_service.update_user_problem_stat") as mock_stat_update,
        ):
            result = submit_code_blame_report(
                self.db,
    ...
    def test_create_problem_generation_failure_returns_user_friendly_error(self):
        ...
        with patch(
            "app.services.code_blame_service._generator.generate_code_blame_problem_sync",
