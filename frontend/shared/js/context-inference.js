...
async function tryResumeReview() {
  ...
  return reviewResume.resumeReviewProblem({
    ...
    applyProblem: async (problem) => {
      ...
      clearFeedback();
      if (elements.loadStatus) {
        elements.loadStatus.textContent = "같은 문제를 다시 열었습니다. 추론 리포트를 다시 작성해 보세요.";
      }
    },
    onError: (error) => {
...
async function loadContextInferenceProblemViaStream() {
  const streamClient = getProblemStreamClient();
...
async function animateContextInferenceProblem(problem) {
  const streamClient = getProblemStreamClient();
  ...
  if (elements.problemTitle) {
    await streamClient.typeText(elements.problemTitle, problem.title || "맥락 추론 문제", {
      minDelay: 10,
      maxDelay: 16,
    });
  }
  if (elements.problemLanguage) {
    await streamClient.typeText(elements.problemLanguage, `언어 ${languageLabel}`, {
      minDelay: 10,
      maxDelay: 16,
    });
  }
  ...
  if (elements.problemType) {
    await streamClient.typeText(elements.problemType, `유형 ${typeLabel}`, {
      minDelay: 10,
      maxDelay: 16,
    });
  }
  if (elements.problemSnippet) {
    await streamClient.revealLines(elements.problemSnippet, problem.snippet || "// 문제 코드가 비어 있습니다.", {
      lineDelay: 70,
    });
  }
  if (elements.problemPrompt) {
    await streamClient.typeText(
      ...
      problem.prompt || "코드 맥락을 추론하고 리포트를 작성해 주세요.",
      {
        minDelay: 10,
        maxDelay: 16,
      }
...
async function handleLoadProblem() {
  ...
  try {
    ...
    try {
      payload = await loadContextInferenceProblemViaStream();
    ...
    if (!payload && allowJsonFallback) {
      ...
      payload = await apiRequest("/platform/context-inference/problem", {
        method: "POST",
        body: {
          language: state.selectedLanguage,
          difficulty: state.selectedDifficulty,
    ...
    if (streamed || usedJsonFallback) {
      await animateContextInferenceProblem(payload);
    } else {
...
function renderProblem(problem) {
  ...
  if (elements.problemPrompt) {
    elements.problemPrompt.textContent = problem.prompt || "코드 맥락을 추론하고 리포트를 작성해 주세요.";
  }
  if (elements.problemLanguage) {
    const languageId = problem.language || state.selectedLanguage;
    const languageLabel = state.languageMap[languageId]?.title || languageId || "-";
  ...
  if (elements.problemType) {
    elements.problemType.textContent = `유형 ${typeLabel}`;
...
function clearProblem() {
  ...
  if (elements.problemPrompt) {
    elements.problemPrompt.textContent = "문제를 받은 뒤 추론 리포트를 작성해 주세요.";
  }
  if (elements.problemLanguage) {
  ...
  if (elements.problemType) {
    elements.problemType.textContent = "유형 -";
...
async function handleSubmitReport(event) {
  ...
  try {
    const payload = await apiRequest("/platform/context-inference/submit", {
      method: "POST",
      body: {
...
function clearFeedback() {
  ...
  if (elements.feedbackSummary) {
    elements.feedbackSummary.textContent = "리포트를 제출하면 AI 요약이 표시됩니다.";
  }
  ...
  if (elements.referenceReport) {
    elements.referenceReport.textContent = "제출 후 공개됩니다.";
  }
}
