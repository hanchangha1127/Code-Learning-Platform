...
async function loadProblem() {
  ...
  try {
    ...
    if (isProblemLoadCancelled(error)) {
      ...
      renderStatusCards(false);
      if (elements.modeState) {
        elements.modeState.textContent = "문제 취소";
      }
      if (elements.loadStatus) {
        elements.loadStatus.textContent = "문제 불러오기를 취소했습니다.";
      }
      updateSubmitButtonState();
      return;
    }
...
function applyStreamingProblemPreview(draft) {
  ...
  if (Array.isArray(draft.files) && draft.files.length) {
    ...
    if (previewFiles.length) {
      ...
      const renderToken = ++state.codeRenderToken;
      renderActiveFileMeta(activeFile, previewFiles.length);
      void renderCodeView(activeFile?.content || "", { animate: false, renderToken });
...
async function animateProblemRender(problem) {
  ...
  if (elements.codeView) {
    await renderCodeView(activeFile?.content || "", { animate: true, renderToken });
  }
  if (renderToken === state.codeRenderToken) {
    setActiveButtons(activeFile?.id || "");
  }
}
...
function renderActiveFile(file) {
  ...
  void renderCodeView(file?.content || "", { animate: false, renderToken });
...
async function renderCodeView(code, { animate, renderToken = state.codeRenderToken } = {}) {
  if (!elements.codeView) {
    return;
  }
