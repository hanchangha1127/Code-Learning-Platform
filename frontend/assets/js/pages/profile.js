...
const FRONTEND_TO_BACKEND_DIFFICULTY = {
  beginner: "easy",
  intermediate: "medium",
  advanced: "hard",
};
const BACKEND_TO_FRONTEND_DIFFICULTY = {
  easy: "beginner",
  medium: "intermediate",
  hard: "advanced",
};
...
} from "../widgets/advanced_history_view.js";
...
function cacheDom() {
  ...
  elements.settingsStatus = document.getElementById("profile-settings-status");
  elements.totalAttempts = document.getElementById("profile-total-attempts");
  elements.accuracy = document.getElementById("profile-accuracy");
  elements.weekAttempts = document.getElementById("profile-week-attempts");
  elements.reviewCount = document.getElementById("profile-review-count");
  elements.goalProgress = document.getElementById("profile-goal-progress");
  elements.goalNote = document.getElementById("profile-goal-note");
  elements.streakDays = document.getElementById("profile-streak-days");
  elements.focusModesCount = document.getElementById("profile-focus-modes-count");
  elements.focusModes = document.getElementById("profile-focus-modes");
  elements.weakTopicsCount = document.getElementById("profile-weak-topics-count");
  elements.weakTopics = document.getElementById("profile-weak-topics");
  elements.reviewLink = document.getElementById("profile-review-link");
  elements.recommendLink = document.getElementById("profile-recommend-link");
...
async function init() {
  ...
  await loadLanguages();
  await loadSettings();
  await Promise.allSettled([loadProfile(), loadHome(), loadGoal()]);
...
function normalizeBackendDifficulty(value) {
  const normalized = normalizeText(value).toLowerCase();
  return BACKEND_TO_FRONTEND_DIFFICULTY[normalized] || normalized || DEFAULT_DIFFICULTY;
}

function normalizeFrontendDifficulty(value) {
  const normalized = normalizeText(value).toLowerCase();
  return FRONTEND_TO_BACKEND_DIFFICULTY[normalized] || FRONTEND_TO_BACKEND_DIFFICULTY[DEFAULT_DIFFICULTY];
}
...
function renderProfileOverview() {
  ...
  if (elements.totalAttempts) {
    elements.totalAttempts.textContent = String(stats.totalAttempts ?? profile.totalAttempts ?? 0);
  }
  if (elements.accuracy) {
    const value = stats.accuracy ?? profile.accuracy;
    elements.accuracy.textContent = value === undefined || value === null ? "-" : `${Math.round(Number(value) || 0)}%`;
  }
  if (elements.weekAttempts) {
    elements.weekAttempts.textContent = String(trend.last7DaysAttempts ?? 0);
  }
  if (elements.reviewCount) {
    elements.reviewCount.textContent = String(reviewQueue.dueCount ?? 0);
  }
  if (elements.streakDays) {
    elements.streakDays.textContent = `${home.streakDays ?? 0}일`;
  }

  const reviewItem = Array.isArray(reviewQueue.items) ? reviewQueue.items[0] : null;
  if (elements.reviewLink) {
    elements.reviewLink.href = reviewItem?.resumeLink || reviewItem?.actionLink || "/analysis.html";
    elements.reviewLink.textContent = reviewQueue.dueCount > 0 ? "대기 복습 열기" : "복습 기록 보기";
  }

  const recommended = Array.isArray(home.recommendedModes) ? home.recommendedModes[0] : null;
  if (elements.recommendLink) {
    elements.recommendLink.href = recommended?.link || "/dashboard.html";
    elements.recommendLink.textContent = recommended?.label ? `${recommended.label} 열기` : "추천 모드 열기";
  }
}
...
function renderGoalSummary() {
  ...
  const weakTopics = Array.isArray(home.weakTopics) ? home.weakTopics.filter(Boolean) : [];
  const focusModes = Array.isArray(goal.focusModes) && goal.focusModes.length
    ? goal.focusModes
    : Array.isArray(home.focusModes)
      ? home.focusModes
      : [];
  const focusTopics = Array.isArray(goal.focusTopics) && goal.focusTopics.length
    ? goal.focusTopics
    : weakTopics;

  if (elements.goalProgress) {
    const completed = Number(dailyGoal.completedSessions ?? 0);
    const target = Number(goal.dailyTargetSessions ?? dailyGoal.targetSessions ?? 0);
    elements.goalProgress.textContent = target > 0 ? `${completed} / ${target}` : "목표 없음";
  }
  if (elements.goalNote) {
    const remaining = Number(dailyGoal.remainingSessions ?? 0);
    if (dailyGoal.achieved) {
      elements.goalNote.textContent = "오늘 목표를 달성했습니다.";
    } else if (remaining > 0) {
      elements.goalNote.textContent = `오늘 목표까지 ${remaining}세션 남았습니다.`;
    } else {
      elements.goalNote.textContent = "오늘 목표 정보를 아직 불러오지 못했습니다.";
    }
  }

  if (elements.focusModesCount) {
    elements.focusModesCount.textContent = String(focusModes.length || 0);
  }
  if (elements.focusModes) {
    elements.focusModes.textContent = focusModes.length
      ? focusModes.map((mode) => formatModeLabel(mode)).join(" · ")
      : "집중 모드가 아직 없습니다.";
  }
  if (elements.weakTopicsCount) {
    elements.weakTopicsCount.textContent = String(focusTopics.length || 0);
  }
  if (elements.weakTopics) {
    elements.weakTopics.textContent = focusTopics.length
      ? focusTopics.join(" · ")
      : "아직 약한 주제가 집계되지 않았습니다.";
  }
}
...
async function loadLanguages() {
  ...
  try {
    ...
    setSettingsStatus("언어 목록을 불러오지 못했습니다.");
  }
}

async function loadSettings() {
  try {
    const settings = await apiRequest("/platform/me/settings");
    const preferredLanguage = normalizeLanguageId(settings?.preferred_language);
    const preferredDifficulty = normalizeBackendDifficulty(settings?.preferred_difficulty);
    const hasLanguage = state.languages.some((lang) => lang.id === preferredLanguage);

    state.selectedLanguage = hasLanguage ? preferredLanguage : getSavedLanguage(state.languages);
    state.difficulty = preferredDifficulty || DEFAULT_DIFFICULTY;
    ...
    renderLanguageOptions();
    renderDifficultyOptions();
    setSettingsStatus("저장한 공통 학습 설정을 모든 모드에서 사용합니다.");
  } catch (error) {
    renderLanguageOptions();
    renderDifficultyOptions();
    setSettingsStatus("서버 설정을 불러오지 못해 브라우저에 저장된 값으로 표시합니다.");
  }
}

async function persistSettings() {
  try {
    setSettingsStatus("공통 설정을 저장하는 중입니다.");
    await apiRequest("/platform/me/settings", {
      method: "PUT",
      body: {
        preferred_language: state.selectedLanguage,
        preferred_difficulty: normalizeFrontendDifficulty(state.difficulty),
      },
    });
    setSettingsStatus("공통 설정이 저장되었습니다. 모든 학습 모드에 바로 반영됩니다.");
  } catch (error) {
    setSettingsStatus(error?.message || "서버 저장에 실패해 브라우저 설정만 유지합니다.");
  }
}

async function loadHome() {
  try {
    state.home = await apiRequest("/platform/home");
  } catch {
    state.home = null;
  }
  renderProfileOverview();
...
async function loadGoal() {
  try {
    state.goal = await apiRequest("/platform/me/goal");
  } catch {
    state.goal = null;
  }
  renderGoalSummary();
...
function getSavedDifficulty() {
  const value = normalizeBackendDifficulty(window.localStorage.getItem(DIFFICULTY_KEY));
...
function formatDifficultyLabel(value) {
  const normalized = normalizeBackendDifficulty(value);
  const found = DIFFICULTY_OPTIONS.find((option) => option.id === normalized);
