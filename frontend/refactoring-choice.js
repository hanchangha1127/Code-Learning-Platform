...
async function handleLoadProblem() {
  ...
  try {
    const payload = await apiRequest("/api/refactoring-choice/problem", {
      method: "POST",
      body: {
...
function renderProblem(problem) {
  ...
  if (elements.problemPrompt) {
    elements.problemPrompt.textContent = problem.prompt || "A/B/C 중 최적안을 선택하고 근거를 작성하세요.";
  }
  ...
  if (elements.problemFacets) {
    ...
    elements.problemFacets.textContent = facets.length ? `facet ${facets.join(", ")}` : "facet -";
  }
...
async function handleSubmitReport(event) {
  ...
  try {
    const payload = await apiRequest("/api/refactoring-choice/submit", {
      method: "POST",
      body: {
...
function renderFeedback(payload) {
  ...
  if (elements.referenceReport) {
    elements.referenceReport.textContent = payload.referenceReport || "모범 메모가 제공되지 않았습니다.";
  }
}
...
function clearProblem() {
  ...
  if (elements.problemTitle) {
    elements.problemTitle.textContent = "문제를 불러오면 최적안을 선택할 수 있습니다.";
  }
  if (elements.problemScenario) {
    elements.problemScenario.textContent = "아직 불러온 시나리오가 없습니다.";
  }
  if (elements.problemPrompt) {
    elements.problemPrompt.textContent = "문제를 받은 뒤 선택 근거를 작성하세요.";
  }
  if (elements.problemLanguage) {
  ...
  if (elements.problemFacets) {
    elements.problemFacets.textContent = "facet -";
  }
