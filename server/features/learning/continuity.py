...
from server.features.learning.catalog import ACTIVE_FOCUS_MODES, MODE_LABELS, MODE_LINKS, infer_mode_from_problem
...
def get_or_create_learning_goal(db: Session, user_id: int) -> UserLearningGoal:
    ...
    goal = UserLearningGoal(
        ...
        focus_modes=["analysis", "code-block", "code-arrange"],
        focus_topics=[],
    )
    db.add(goal)
    try:
...
def update_learning_goal(
    ...
    *,
    ...
    if focus_modes is not None:
        goal.focus_modes = [mode for mode in focus_modes if mode in ACTIVE_FOCUS_MODES]
...
def serialize_learning_goal(goal: UserLearningGoal) -> dict[str, Any]:
    ...
    focus_modes = [mode for mode in _normalize_string_list(goal.focus_modes) if mode in ACTIVE_FOCUS_MODES]
    return {
...
def build_learning_home(
    *,
    ...
    return {
        ...
        "focusModes": [mode for mode in _normalize_string_list(goal.focus_modes) if mode in ACTIVE_FOCUS_MODES],
        "focusTopics": _normalize_string_list(goal.focus_topics),
...
def _mode_from_problem(problem: Problem) -> str:
    ...
    return infer_mode_from_problem(problem=problem, problem_payload=problem_payload)
