...
from server.schemas.learning_continuity import (
    LearningHistoryPageRead,
    LearningHomeRead,
    ReviewQueueRead,
    ReviewResumeRead,
)
from server.schemas.report import LearningSolutionReportRead
...
from server.schemas.runtime import (
...
def get_languages() -> dict:
    return {"languages": learning_service.list_public_languages()}


@router.get("/profile")
def get_profile(current: User = Depends(get_current_user)) -> dict:
    return learning_service.get_public_profile(current.username)
...
def get_home(
    ...
    history = learning_service.get_public_history(current.username, limit=HISTORY_DEFAULT_LIMIT)
    profile = learning_service.get_public_profile(current.username)
    me = learning_service.get_public_me(current)
...
def get_report(
    ...
    try:
        return learning_service.get_public_report(current.username, current.id, db)
...
def get_history(
    ...
    return learning_service.get_public_history_page(current.username, limit=limit)


@router.get("/learning/memory")
def get_memory(current: User = Depends(get_current_user)) -> dict:
    return {"memory": learning_service.get_public_memory(current.username)}
