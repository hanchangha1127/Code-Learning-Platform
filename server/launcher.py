...
def _resolve_secret_file_path(base_dir: Path, raw_path: str | None) -> Path | None:
...
def _load_secret_file_value(base_dir: Path, raw_path: str | None) -> str | None:
    secret_path = _resolve_secret_file_path(base_dir, raw_path)
...
def _load_env_or_secret_value(env_path: Path, key: str) -> str | None:
    direct_value = os.getenv(key)
    if direct_value is None:
        direct_value = _load_env_value(env_path, key)
    if direct_value is not None and direct_value.strip():
        return direct_value.strip()

    file_key = f"{key}_FILE"
    file_value = os.getenv(file_key)
    if file_value is None:
        file_value = _load_env_value(env_path, file_key)
    return _load_secret_file_value(env_path.parent, file_value)


def _compose_runtime_env(project_dir: Path) -> dict[str, str]:
    env_path = project_dir / ".env"
    env = os.environ.copy()
    for required_key in ("DB_PASSWORD", "JWT_SECRET"):
        resolved = _load_env_or_secret_value(env_path, required_key)
        if resolved:
            env[required_key] = resolved
    return env
...
def _run_local_runtime_server(host: str, port: int, workers: int) -> int:
    from server.runtime_server import run_application_server
...
def _resolve_admin_urls(
    *,
    admin_host: str,
    admin_port: int,
    wait_timeout: int,
    https_runtime: dict[str, int | bool],
    default_admin_port: int,
) -> tuple[str, str]:
    if bool(https_runtime["enabled"]):
        https_port = admin_port
        if admin_port == default_admin_port:
            https_port = int(https_runtime["https_public_port"])
        https_netloc = _format_public_netloc(admin_host, https_port, https_enabled=True)
        https_health_url = f"https://{https_netloc}/health"
        if _wait_for_url(https_health_url, wait_timeout, insecure_https=True):
            redirect_port = int(https_runtime["http_redirect_port"])
            redirect_url = f"http://{admin_host}:{redirect_port}/health"
            _wait_for_http_redirect(
                redirect_url,
                expected_prefix=f"https://{https_netloc}",
                timeout_seconds=wait_timeout,
            )
            return https_health_url, f"https://{https_netloc}/admin.html"

    http_netloc = _format_public_netloc(admin_host, admin_port, https_enabled=False)
    http_health_url = f"http://{http_netloc}/health"
    _wait_for_url(http_health_url, wait_timeout, insecure_https=False)
    return http_health_url, f"http://{http_netloc}/admin.html"
...
def _run_docker_compose(
    ...
    if services:
        cmd.extend(services)
    return subprocess.run(cmd, cwd=project_dir, env=_compose_runtime_env(project_dir)).returncode
...
def _read_compose_status(
    ...
    result = subprocess.run(
        cmd,
        cwd=project_dir,
        env=_compose_runtime_env(project_dir),
        capture_output=True,
        text=True,
        encoding="utf-8",
        errors="replace",
...
def _print_service_failure_diagnostics(
    ...
    compose_env = _compose_runtime_env(project_dir)
    for service in sorted(missing_services):
        print(f"[launcher] Showing last logs for '{service}'...")
        subprocess.run(compose_cmd + ["logs", "--tail=80", service], cwd=project_dir, env=compose_env)
...
def main() -> int:
    ...
    if not args.local and not _is_in_docker():
        ...
        if detach and not args.no_open_admin:
            ...
            _, admin_url = _resolve_admin_urls(
                admin_host=args.admin_host,
                admin_port=args.admin_port,
                wait_timeout=args.wait_timeout,
                https_runtime=https_runtime,
                default_admin_port=parser.get_default("admin_port"),
            )
            if not admin_key:
