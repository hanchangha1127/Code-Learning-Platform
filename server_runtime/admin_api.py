...
def _shutdown_enable_guidance() -> str:
    return (
        ...
        "or `docker compose -f docker-compose.yml -f docker-compose.docker-socket.yml up -d --build`."
    )
...
class _AdminKeyGuard:
    ...
    def require(
        ...
        with self._lock:
            ...
            # Allow immediate recovery with a correct key even if this client is blocked.
            is_valid = bool(provided) and hmac.compare_digest(provided, expected)
            if is_valid:
    ...
    *,
    ...
    def admin_page() -> Response:
        if admin_file.exists():
            return FileResponse(str(admin_file), media_type="text/html; charset=utf-8")
    ...
    def admin_shutdown(
        ...
        background_tasks.add_task(_shutdown_runtime_stack)
